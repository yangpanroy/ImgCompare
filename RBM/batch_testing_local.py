...
def make_dataset(internal_img1_dir, internal_img2_dir):
...
if __name__ == '__main__':
    ...
    predictions_dir = 'E:/result/SupervisedModel/test/prediction/local/'  # 存/放预测结果的路径
    ...
    for file_name in f_list1:
        if os.path.splitext(file_name)[1] == '.png':
            ...
            if file_name in f_list2:
                ...
                print("\n==============开始制作dataset=============")
                dataset = make_dataset(img1_path, img2_path)
                print("==============开始读取模型并批量进行预测=============")
                ...
                del predictions
                cv2.imwrite(predictions_dir + file_name.split('.')[0] + '.jpg', predictions_img)
                del predictions_img
                file_num = file_num + 1
