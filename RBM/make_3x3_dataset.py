...
for dim in range(3):
    ...
    if (height == height2) and (width == width2):
        ...
        if SPLIT_DATA == 1:
            np.save("/media/files/yp/rbm/dataset/train07_3x3_" + dim_name + ".npy", content[:train_lenth])
            np.save("/media/files/yp/rbm/dataset/valid07_3x3_" + dim_name + ".npy", content[train_lenth:])
        else:
            np.save("/media/files/yp/rbm/dataset/test02_3x3_" + dim_name + ".npy", content)
