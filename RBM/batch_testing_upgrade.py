...
def make_dataset(internal_img1_dir, internal_img2_dir, kernel_width=5):
    ...
    content_RGB = np.reshape(content_RGB, (height*width, kernel_width*kernel_width*6))
    return content_RGB
...
if __name__ == '__main__':
    os.environ["CUDA_VISIBLE_DEVICES"] = '3'  # 指定第二块GPU可用
    ...
    img1_dir = '/media/files/yp/rbm/yinchuansanqu/2015/xingqing/'  # 时相1图像的路径
    img2_dir = '/media/files/yp/rbm/yinchuansanqu/2016/xingqing/raw/'  # 时相2图像的路径
    predictions_dir = '/media/files/yp/rbm/yinchuansanqu/predictions/15-16/xingqing/build/'  # 存/放预测结果的路径
    ...
    for file_name in f_list1:
        if os.path.splitext(file_name)[1] == '.TIF':
            ...
            if another_file_name in f_list2 and third_name not in f_list3:
                ...
                print "\n==============开始制作dataset============="
                dataset = make_dataset(img1_path, img2_path, kernel_width)
                print "==============开始读取模型并批量进行预测============="
